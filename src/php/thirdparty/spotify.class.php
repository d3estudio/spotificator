...
class Spotify {
  ...
  public static function searchTrackGetFirstID($title, $page = 1) {
    $tracks = self::searchTrack($title, $page);
    if($tracks AND isset($tracks->tracks) AND isset($tracks->tracks[0]) AND isset($tracks->tracks[0]->href)) {
      return str_replace('spotify:track:', '', $tracks->tracks[0]->href);
    }
    else {
      return false;
    }
